...
ggsave("output/allocation_rule_facilities_map.png", fac_map, width = 10, height = 6)
...
write.xlsx(list_of_datasets,"output/summary_tables/allocation_rule_summary_tables_national.xlsx", overwrite = TRUE)
...
for (i in 1:NROW(facilities_urban)){
  ...
  # get the national urban area level averages (rural==0)
  ...
  # get the urban area level averages in that state
  state <- table %>% filter(State==facilities_urban[i,]$`State_Prov`)
  for (v in 1:length(comparison_vars)) {
    summary_table[v,"State Average"] = sum(state$pop*state[,comparison_vars[v]],na.rm=T)/sum(state$pop,na.rm=T)
    a = (state$pop*state[,comparison_vars[v]])/state$pop
    summary_table_sd[v,"State Average SD"] = sqrt(sum((a-mean(a, na.rm=TRUE))^2/(length(a)-1), na.rm=TRUE))
  ...
  # export
  list_of_datasets <- list("Means" = summary_table, "Standard Deviations" = summary_table_sd)
  write.xlsx(list_of_datasets,paste0("output/summary_tables/urban/allocation_rule_summary_tables_",facility,".xlsx"), overwrite = TRUE)
  
}

####################################################
###############  Conduct proximity analysis by plant - pellets
####################################################

facilities_pellets <- facilities_map %>% filter(Type == "pellet")

for (i in 1:NROW(facilities_pellets)){
  
  facility <- paste0(facilities_pellets[i,]$Label)
  communities = st_buffer(facilities_pellets[i,],dist=1*1609.34) 
  communities_3mi = st_buffer(facilities_pellets[i,],dist=3*1609.34) 
  communities_5mi = st_buffer(facilities_pellets[i,],dist=5*1609.34) 
  communities_10mi = st_buffer(facilities_pellets[i,],dist=10*1609.34) 
  ...
  # get the national pellet mill averages
  for (v in 1:length(comparison_vars)) {
    summary_table[v,"National Average"] = sum(table$pop*table[,comparison_vars[v]],na.rm=T)/sum(table$pop,na.rm=T)
    a = (table$pop*table[,comparison_vars[v]])/table$pop
    summary_table_sd[v,"National Average SD"] = sqrt(sum((a-mean(a, na.rm=TRUE))^2/(length(a)-1), na.rm=TRUE))
  ...
  # get the pellet mills averages in that state
  state <- table %>% filter(State==facilities_pellets[i,]$`State_Prov`)
  for (v in 1:length(comparison_vars)) {
    summary_table[v,"Pellet Mills (State Average)"] = sum(state$pop*state[,comparison_vars[v]],na.rm=T)/sum(state$pop,na.rm=T)
    a = (state$pop*state[,comparison_vars[v]])/state$pop
    summary_table_sd[v,"Pellet Mills (State Average) SD"] = sqrt(sum((a-mean(a, na.rm=TRUE))^2/(length(a)-1), na.rm=TRUE))
  }
  ...
  # export
  list_of_datasets <- list("Means" = summary_table, "Standard Deviations" = summary_table_sd)
  write.xlsx(list_of_datasets,paste0("output/summary_tables/pellets/allocation_rule_summary_tables_",facility,".xlsx"), overwrite = TRUE)
